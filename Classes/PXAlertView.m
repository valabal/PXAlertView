...
static const CGFloat AlertViewButtonHeight = 48.0;
         ...
         completion:(PXAlertViewCompletionBlock)completion
{
    ...
    if (self) {
        ...
        CGRect lineRect = CGRectMake(0, messageLabelY, AlertViewWidth, 1);
        ...
        CGRect frameLabel = [self adjustLabelFrameHeight:self.messageLabel];
        if(frameLabel.size.height > 0)
        {frameLabel.size.height = frameLabel.size.height + 40;}
        self.messageLabel.frame = frameLabel;
...
- (void)resizeViews
{
    ...
    for (UIView *view in [self.alertView subviews]) {
        if ([view class] != [UIButton class] && view.tag != LINE_VIEW_TAG) {
...
- (void)dismiss:(id)sender animated:(BOOL)animated
{
    ...
    [UIView animateWithDuration:(animated ? 0.2 : 0) animations:^{
        ...
        if (self.completion) {
            ...
            if([sender isKindOfClass:[UIButton class]] && ((UIButton *)sender).tag == 4567){
                cancelled = YES;
            }
...
- (NSInteger)addButtonWithTitle:(NSString *)title
{
    ...
    CGFloat alertWidth = AlertViewWidth - 20;
    CGFloat originX = 10;
    
    CGFloat alertHeight = AlertViewButtonHeight - 10;
    
    
    if (!self.cancelButton) {
        button.titleLabel.font = [UIFont boldSystemFontOfSize:17];
        self.cancelButton = button;
        self.cancelButton.frame = CGRectMake(originX, self.buttonsY, alertWidth, alertHeight);
        ...
        CGFloat lastButtonYOffset = self.cancelButton.frame.origin.y + alertHeight;
        self.cancelButton.frame = CGRectMake(originX, lastButtonYOffset, alertWidth , alertHeight);
        
        CALayer *lineLayer = [self lineLayer];
        lineLayer.frame = CGRectMake(originX, lastButtonYOffset, alertWidth, AlertViewLineLayerWidth);
        [self.alertView.layer addSublayer:lineLayer];
        ...
        if ([self.buttons count] == 2) {
            ...
            CALayer *lineLayer = [self lineLayer];
            lineLayer.frame = CGRectMake(originX, self.buttonsY + alertHeight, alertWidth, AlertViewLineLayerWidth);
            [self.alertView.layer addSublayer:lineLayer];
            lastButton.frame = CGRectMake(originX, self.buttonsY + alertHeight, alertWidth, alertHeight);
            self.cancelButton.frame = CGRectMake(originX, self.buttonsY, alertWidth, alertHeight);
        }
        CGFloat lastButtonYOffset = lastButton.frame.origin.y + alertHeight;
        button.frame = CGRectMake(originX, lastButtonYOffset, alertWidth, alertHeight);
    } else {
        self.verticalLine = [self lineLayer];
        
        CGFloat width = (AlertViewWidth - 30)/2;
        
        self.verticalLine.frame = CGRectMake(originX+AlertViewWidth/2, self.buttonsY, AlertViewLineLayerWidth, alertHeight);
        [self.alertView.layer addSublayer:self.verticalLine];
        
        button.frame = CGRectMake((originX+AlertViewWidth)/2, self.buttonsY, width, alertHeight);
        self.otherButton = button;
        self.cancelButton.frame = CGRectMake(originX, self.buttonsY, width, alertHeight);
